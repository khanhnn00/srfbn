...
def get_imgs(path):
    ...
    # print(type(lr))
    cv2.imshow('lr', lr.clip(0, 255).astype(np.uint8))
    cv2.imshow('hr', hr.clip(0, 255).astype(np.uint8))
    cv2.imshow('hr_x', hr_x.clip(0, 255).astype(np.uint8))
    hr_x_dn = add_noise(hr, noise='G30')
    lr_dn = misc.imresize(hr_x_dn, 1 / 4, interp='bicubic')
    cv2.imshow('hr_x_dn', hr_x_dn.clip(0, 255).astype(np.uint8))
    cv2.imshow('lr_dn', lr_dn.clip(0, 255).astype(np.uint8)
    cv2.waitKey(0)
